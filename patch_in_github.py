...
def update_addon_version():
    ...
    match = re.search(r'SOFTVERSION\s*=\s*(\d+\.\d+)', content)
    if not match:
        print("Error: SOFTVERSION not found in utils.py")
        return None, None
    
    current_ver = float(match.group(1))
    new_ver = round(current_ver + 0.1, 1)
    
    new_content = re.sub(
        r'SOFTVERSION\s*=\s*\d+\.\d+',
        f'SOFTVERSION = {new_ver}',
        content
    )
